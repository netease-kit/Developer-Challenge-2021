	...
	<view class="rf-index">
		...
		<view :style="{
			...
			<block v-if="currentCate === 0">
				...
				<!--新闻通知-->
				<rf-swiper-slide v-if="announceList.length > 0" :list="announceList" class="rf-skeleton" @navTo="navTo('/pages/mail/notice/notice')">
				...
				<view class="hot-recommend">
					<view class="left">
						<image class="hot-recommend-image"  src="https://img2.epetbar.com/nowater/2019-04/25/15/e5d2d12b1ce0b2f223338fc76b268ff2.jpg"></image>
					</view>
					<view class="right">
						<image class="hot-recommend-image" @tap.stop="navTo(hotRecommendList[1].url)" src="https://img2.epetbar.com/nowater/showcase/2020/04/08/092251686.jpg@!water"></image>
						<image class="hot-recommend-image" @tap.stop="navTo(hotRecommendList[2].url)" src="https://img2.epetbar.com/nowater/showcase/2020/04/06/001011173.jpg@!water"></image>
					</view>
				</view>
				<!--新品上市-->
				<rf-floor-index
...
<script>
	...
	export default {
		...
		onShareAppMessage() {
			let shareParams = { title: this.share.share_title || `欢迎来到${this.appName}`, path: '/pages/mail/index' };
			return shareParams;
		},
		filters: {
		...
		methods: {
			...
			navToSearch() {
				this.$mRouter.push({
					route: `/pages/mail/search/search?data=${JSON.stringify(this.search)}`
				});
			},
			// 跳至广告图指定页面
			...
			async getIndexList(type) {
				/* await this.$http
					...
					const data = this.indexList.data
					// 首页参数赋值
					this.initIndexData(data);
					this.loading = false;
			},
			// 首页参数赋值
			initIndexData(data) {
