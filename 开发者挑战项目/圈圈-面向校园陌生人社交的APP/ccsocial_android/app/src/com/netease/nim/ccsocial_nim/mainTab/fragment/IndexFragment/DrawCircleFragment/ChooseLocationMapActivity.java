...
public class ChooseLocationMapActivity extends UI implements OnCameraChangeListener, OnClickListener, NimLocationListener {
    ...
    public static void start(Context context, LocationProvider.Callback callback) {
        ChooseLocationMapActivity.callback = callback;
        context.startActivity(new Intent(context, ChooseLocationMapActivity.class));
    }
...
//        finish();
        ...
        public void onGeoCoderResult(NimLocation location) {
            if (latitude == location.getLatitude() && longitude == location.getLongitude()) { // 响应的是当前查询经纬度
                if (location.hasAddress()) {
                    ChooseLocationMapActivity.this.addressInfo = location.getFullAddr();
                } else {
    ...
    private Runnable runable = new Runnable() {
        ...
        @Override
        public void run() {
            ChooseLocationMapActivity.this.addressInfo = getString(R.string.location_address_unkown);
            setPinInfoPanel(true);
        }
    };
